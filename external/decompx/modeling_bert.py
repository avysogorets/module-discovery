...
from .utils import optimized_transform
...
def output_builder(
        ...
        mode,
        norm='L2',
        aggregation_in_layer=None,
        mixing_in_layer=None,
        output_tensor=None,
        mixing_in_cls=False):
    # attribution_tensor: B x H? x S x S x D
    # output_tensor:      B x H? x S x D
    assert mode in ['absolute', 'relative'], f"unknown aggregation mode {mode}"
    if mixing_in_layer is None:
        assert aggregation_in_layer in ['norm', 'dot'], f"invalid aggregation in layer {aggregation_in_layer}"
    if aggregation_in_layer is None:
        assert mixing_in_layer in ['angle', 'dot'], f"invalid mixing in layer {mixing_in_layer}"
    assert mixing_in_layer is None or aggregation_in_layer is None, f"both aggregation and mixing in layer provided"
    assert norm in ['L2', 'ALTI'], f"unknown norm {norm}"
    if aggregation_in_layer == 'norm':
        if norm == 'L2':
            tensor_1 = torch.norm(attribution_tensor, dim=-1)
            if mode == 'absolute':
                result = tensor_1
            if mode == 'relative':
                Z = torch.norm(output_tensor, dim=-1)[...,None]
                Z[Z == 0] = 1
                result = tensor_1 / Z
        if norm == 'ALTI':
            # ALTI is always relative by design
            distances = torch.norm(output_tensor[...,None,:] - attribution_tensor, p=1, dim=-1)
            contributions = torch.clamp(torch.norm(output_tensor, p=1, dim=-1)[...,None]-distances, min=0)
            contributions = contributions / (torch.sum(contributions, dim=-1, keepdim=True) + 1e-10)
            result = contributions
    if aggregation_in_layer == 'dot':
        dot_tensor = (attribution_tensor * output_tensor[...,None,:]).sum(-1)
        if mode == 'absolute':
            result = dot_tensor
        if mode == 'relative':
            vector_norm_1 = torch.norm(attribution_tensor, dim=-1)
            vector_norm_2 = torch.norm(output_tensor, dim=-1)[...,None]
            Z = vector_norm_1 * vector_norm_2
            Z[Z == 0] = 1
            result = dot_tensor / Z
    if mixing_in_layer == 'dot':
        if mixing_in_cls:
            # ... x 1 x S x S: <A0k, A0j>
            dot_tensor = (attribution_tensor[...,[0],:,:] @ attribution_tensor[...,[0],:,:].transpose(-2,-1))
        else:
            # ... x S x S x S: <Aik, Aij>
            dot_tensor = (attribution_tensor @ attribution_tensor.transpose(-2,-1))
        if mode == 'absolute':
            result = dot_tensor.sum(-3)
        if mode == 'relative':
            vector_norm = torch.norm(output_tensor, dim=-1)
            sum_norm = vector_norm.sum(-1)[...,None]
            sum_norm[sum_norm == 0] = 1
            weight_tensor = vector_norm / sum_norm
            weight_tensor = weight_tensor[...,None,None]
            result = (dot_tensor * weight_tensor).sum(-3)
    if mixing_in_layer == 'angle':
        if mixing_in_cls:
            # ... x 1 x S x S: <A0k, A0j>
            dot_tensor = (attribution_tensor[...,[0],:,:] @ attribution_tensor[...,[0],:,:].transpose(-2,-1))
        else:
            # ... x S x S x S: <Aik, Aij>
            dot_tensor = (attribution_tensor @ attribution_tensor.transpose(-2,-1))
        vector_norm_1 = torch.norm(attribution_tensor, dim=-1)[...,:,None]
        vector_norm_2 = torch.norm(attribution_tensor, dim=-1)[...,None,:]
        Z = vector_norm_1 * vector_norm_2
        Z[Z == 0] = 1
        cos_tensor = dot_tensor / Z
        del dot_tensor, Z
        if mode == 'absolute':
            cos_tensor = cos_tensor.sum(-3)
            result = cos_tensor
        if mode == 'relative':
            vector_norm = torch.norm(output_tensor, dim=-1)
            sum_norm = vector_norm.sum(-1)[...,None]
            ...
            weight_tensor = vector_norm / sum_norm
            weight_tensor = weight_tensor[...,None,None]
            cos_tensor = (cos_tensor * weight_tensor).sum(-3)
            result = cos_tensor
    return ((result, attribution_tensor),)


"""
def output_builder(input_vector, output_mode, attention_output=None, norm=NORM_L2):
    if output_mode is None:
        ...
        if norm == NORM_L2:
            ...
            distances = torch.norm(attention_output[...,None,:] - input_vector, p=1, dim=-1)
            contributions = torch.max(torch.tensor(0.), torch.norm(attention_output, p=1, dim=-1)[...,None] - distances)
            contributions = contributions / (torch.sum(contributions, dim=-1, keepdim=True) + 1e-10)
            return (contributions,)
    elif output_mode == "both":
        # We should only enter here in all cases.
        if norm == NORM_L2:
            ...
            contributions = torch.max(torch.tensor(0.), torch.norm(attention_output, p=1, dim=-1)[:,:,None] - distances)
            contributions = contributions / (torch.sum(contributions, dim=-1, keepdim=True) + 1e-10)
            return ((contributions, input_vector),)
        
    elif output_mode == "distance_based": # ALTI?
...
class BertLayer(nn.Module):
    ...
    def ln_decomposer(self, attribution_vectors, pre_ln_states, gamma, beta, eps, include_biases=True, bias_decomp_type="absdot"):
        ...
        normalized_layer = torch.einsum('bskd,d->bskd', normalized_layer,
                                         gamma)  # (batch, seq_len, seq_len, all_head_size)
        
        if include_biases:
            return self.bias_decomposer(beta, normalized_layer, bias_decomp_type=bias_decomp_type)
        else:
    ...
    def ffn_decomposer(self, attribution_vectors, intermediate_hidden_states, intermediate_output, include_biases=True, approximation_type="GeLU_LA", bias_decomp_type="absdot"):
        ...
        # post_second_layer = post_act_first_layer @ self.output.dense.weight.T
        if include_biases:
    ...
    def forward(
        ...
        if decompx_ready:
            ...
            # Artem Vysogorets: Add that for headlevel NBA support: 
            context_layer = torch.matmul(attention_probs, value_layer)
            context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
            new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
            context_layer = context_layer.view(new_context_layer_shape)
            h_size = self.attention_head_size
            num_heads = self.num_attention_heads
            attention_output_dense = []
            WoT = self.attention.output.dense.weight.data.T
            bo = self.attention.output.dense.bias.data
            for h in range(num_heads):
                t = context_layer[:,:,h*h_size:(h+1)*h_size]
                t = t @ WoT[h*h_size:(h+1)*h_size]
                t = t + bo / num_heads 
                t = t[:,:,None,:]
                attention_output_dense.append(t)
            attention_output_dense = torch.concatenate(attention_output_dense, dim=2)
            attention_output_dense = attention_output_dense.permute(0,2,1,3)
            # this tensor has shape B x H x S x D (post-dense head-wise hidden states)

            headmixing_weight = self.attention.output.dense.weight.view(self.all_head_size, self.num_attention_heads,
                                      self.attention_head_size)
            if decomposed_value_layer is None or decompx_config.aggregation_in_encoder != "vector":
                if decompx_config.memory_optimized: # Added by Artem Vysogorets
                    summed_weighted_layer = optimized_transform(attention_probs, value_layer, headmixing_weight, chunk_size=16)
                else:
                ...
                residual_weighted_layer = torch.einsum('sk,bsd->bskd', torch.eye(hidden_shape[1], **tensor_kwargs), hidden_states)

                # Make matrix of summed weighted vector + residual vectors
                residual_weighted_layer += summed_weighted_layer
                accumulated_bias = self.attention.output.dense.bias
            else:
                if decompx_config.memory_optimized: # Added by Artem Vysogorets
                    summed_weighted_layer = optimized_transform(attention_probs, decomposed_value_layer, headmixing_weight, chunk_size=16)
                else:
                    transformed_layer = torch.einsum('bhsqv,dhv->bhsqd', decomposed_value_layer, headmixing_weight)
                    weighted_layer = torch.einsum('bhks,bhsqd->bhkqd', attention_probs, transformed_layer)  # attention_probs(Q*K^t) * V * W^o
                    summed_weighted_layer = weighted_layer.sum(dim=1)  # sum over heads
            ...
            if decompx_config.include_LN1:
                output_tensor = attention_output # added by Artem Vysogorets
                ...
                output_tensor = pre_ln_states

            if decompx_config.include_FFN:
                output_tensor = pre_ln2_states # added by Artem Vysogorets
            ...
            if decompx_config.include_LN2:
                output_tensor = layer_output # added by Artem Vysogorets
            ...
            # stop after attention.dense
            if not decompx_config.include_res:
                if decompx_config.collection_level == HEAD:
                    attribution_tensor = weighted_layer
                    output_tensor = attention_output_dense
                else:
                    attribution_tensor = summed_weighted_layer
                    output_tensor = attention_output_dense.sum(1)
            # otherwise, all other includes are taken care of.
            # the correct attribute tensor is in post_ln2_layer
            # despite the name.
            else:
                attribution_tensor = post_ln2_layer


            # modified by Artem Vysogorets
            new_outputs = DecompXOutput(
                layer=output_builder(
                    attribution_tensor=attribution_tensor,
                    mode=decompx_config.mode,
                    norm=decompx_config.norm,
                    mixing_in_cls=decompx_config.mixing_in_cls,
                    aggregation_in_layer=decompx_config.aggregation_in_layer,
                    mixing_in_layer=decompx_config.mixing_in_layer,
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        aggregated_encoder_scores = None # added by Fayyaz / Modarressi
        ...
        if decompx_config and decompx_config.output_all_layers:
            ...
            all_decompx_outputs = DecompXOutput(
                ...
                aggregated=(), #if decompx_config.output_aggregated else None,
            )
        else:
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            hidden_states = layer_outputs[0] # BLD
            if use_cache:
            ...
            # added by Fayyaz / Modarressi # modified by Artem Vysogorets
            if decompx_config:
                decompx_output = layer_outputs[1]
                
                encoder_scores = decompx_output.layer[0][0]
                if decompx_config.aggregation_in_encoder == "rollout":
                    bin_mask_rows = torch.exp(attention_mask).view((-1, attention_mask.shape[-1], 1))
                    bin_mask_cols = torch.exp(attention_mask).view((-1, 1, attention_mask.shape[-1]))
                    bin_mask = bin_mask_cols * bin_mask_rows
                    encoder_scores *= bin_mask
                    if aggregated_encoder_scores is None: # first layer
                        aggregated_encoder_scores = encoder_scores
                    else:
                        aggregated_encoder_scores = encoder_scores @ aggregated_encoder_scores
                    mean = ((aggregated_encoder_scores * bin_mask).sum(-1) / (bin_mask.sum(-1)))[...,None]
                    centered = torch.nan_to_num((aggregated_encoder_scores - mean)*bin_mask, 0)
                    std = torch.sqrt(((centered**2).sum(-1)) / (bin_mask.sum(-1)-1))[...,None]
                    aggregated_encoder_scores = torch.nan_to_num(((aggregated_encoder_scores - mean) / std)*bin_mask, 0)
                    decompx_output.aggregated = (aggregated_encoder_scores,)
                elif decompx_config.aggregation_in_encoder == "vector":
                    aggregated_encoder_vectors = decompx_output.layer[0][1]
                    decompx_output.aggregated = (encoder_scores,)
                else:
                    decompx_output.aggregated = (encoder_scores,)
                ...
                if decompx_config.output_all_layers:
                    ...
                    #if decompx_config.include_classifier_w_pooler and decompx_config.aggregation_in_encoder == "vector":
        ...
        if not return_dict:
            # Our return is here. We only return if entry is not None
            # so don't set all_decompx_outputs to not None if you don't mean it
            return tuple(
                v
                for v in [
                    hidden_states,
                ...
                if v is not None # This is why we take the last element in DecompX (when output_all_layers is None). In that case, the last element is decompx_output.
            )
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        # if decompx_ready:
        if decompx_ready and pooled_output: # Added by Artem Vysogorets
            # We never enter here so don't bother
        ...
        if not return_dict:
            ...
            # Changed by Artem Vysogorets: returning embeddings and token attribution matrix only
            # (DecompX output cannot be aggregated by DataParallel and is not needed in this project)
            return  encoder_outputs[0], encoder_outputs[-1].aggregated
...
class BertSelfAttentionViT(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
...
class BertSelfOutputViT(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        #self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(self, hidden_states: torch.Tensor, decompx_ready=False):  # added by Fayyaz / Modarressi
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        # hidden_states = self.LayerNorm(hidden_states + input_tensor)
        #pre_ln_states = hidden_states + input_tensor  # added by Fayyaz / Modarressi
...
class BertAttentionViT(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = BertSelfAttentionViT(config, position_embedding_type=position_embedding_type)
        self.output = BertSelfOutputViT(config)
        self.pruned_heads = set()
    ...
    def forward(
        ...
        if decompx_ready:
            ...
            #attention_output, pre_ln_states = attention_output
            #outputs = (attention_output, attention_probs,) + (value_layer, decomposed_value_layer, pre_ln_states)  # add attentions and norms if we output them
            outputs = (attention_output, attention_probs,) + (value_layer, decomposed_value_layer)  # add attentions and norms if we output them
            return outputs
        # -------------------------------
...
class BertIntermediateViT(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.intermediate_size)
        if isinstance(config.hidden_act, str):
...
class BertLayerViT(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = BertAttentionViT(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = BertAttentionViT(config, position_embedding_type="absolute")
        self.intermediate = BertIntermediateViT(config)
        self.output = BertOutputViT(config)
    ...
    def forward(
        ...
        if (attribution_vectors is None or decompx_config.aggregation_in_encoder != "vector") and decompx_config.include_LN1:
            hidden_shape = hidden_states.size()  # (batch, seq_length, all_head_size)
            tensor_kwargs = {
                'device': hidden_states.device,
                'dtype': hidden_states.dtype}
            attribution_vectors = torch.einsum('sk,bsd->bskd', torch.eye(hidden_shape[1], **tensor_kwargs), hidden_states)

        pre_ln1_states = hidden_states
        post_ln1_states = self.LayerNorm_1(hidden_states)
        if decompx_config.include_LN1:
            #output_tensor = attention_output # added by Artem Vysogorets
            post_ln1_layer = self.ln_decomposer(
                attribution_vectors=attribution_vectors,
                pre_ln_states=pre_ln1_states,
                gamma=self.LayerNorm_1.weight.data,
                beta=self.LayerNorm_1.bias.data,
                eps=self.LayerNorm_1.eps,
                include_biases=decompx_config.include_biases,
                bias_decomp_type=bias_decomp_type)
        else:
            post_ln1_layer = attribution_vectors
        ...
        pre_ln2_states = attention_output + pre_ln1_states
        post_ln2_states = self.LayerNorm_2(pre_ln2_states)
        intermediate_output, pre_act_hidden_states = self.intermediate(post_ln2_states, decompx_ready=decompx_ready)
        # layer_output, pre_ln2_states = self.output(intermediate_output, attention_output, decompx_ready=decompx_ready)
        layer_output = self.output(intermediate_output)
        layer_output += pre_ln2_states # ViT
        if decompx_ready:
            # attention_probs, value_layer, decomposed_value_layer, pre_ln_states = outputs
            attention_probs, value_layer, decomposed_value_layer = outputs
            ...
            headmixing_weight = self.attention.output.dense.weight.view(self.all_head_size, self.num_attention_heads,
                                      self.attention_head_size)
            #if decomposed_value_layer is None or decompx_config.aggregation_in_encoder != "vector":
            if decomposed_value_layer is None: # if and only if attribution_vectors are None (not the case if include_LN1)
                assert attribution_vectors is None
                if decompx_config.memory_optimized: # Added by Artem Vysogorets
                    summed_weighted_layer = optimized_transform(attention_probs, value_layer, headmixing_weight, chunk_size=16)
                else:
                    transformed_layer = torch.einsum('bhsv,dhv->bhsd', value_layer, headmixing_weight)  # V * W^o  (z=(qk)v)
                ...
                residual_weighted_layer = summed_weighted_layer + attribution_vectors # these are guaranteed to be not None.
                accumulated_bias = torch.matmul(self.attention.output.dense.weight, self.attention.self.value.bias) + self.attention.output.dense.bias
            ...
            if decompx_config.include_LN2:
                output_tensor = post_ln2_states # just added
                post_ln2_layer = self.ln_decomposer(
                    attribution_vectors=residual_weighted_layer,
                    pre_ln_states=pre_ln2_states,
                    gamma=self.LayerNorm_2.weight.data,
                    beta=self.LayerNorm_2.bias.data,
                    eps=self.LayerNorm_2.eps,
                    include_biases=decompx_config.include_biases,
                    bias_decomp_type=bias_decomp_type)
            else:
                post_ln2_layer = residual_weighted_layer
                output_tensor = pre_ln2_states


            if decompx_config.include_FFN:
                output_tensor = layer_output # added by Artem Vysogorets
                post_ffn_layer = self.ffn_decomposer_fast if decompx_config.FFN_fast_mode else self.ffn_decomposer(
                    attribution_vectors=post_ln2_layer,
                    intermediate_hidden_states=pre_act_hidden_states,
                    intermediate_output=intermediate_output,
                    approximation_type=decompx_config.FFN_approx_type,
                    include_biases=decompx_config.include_biases,
                ...
                #pre_ln2_layer = post_ln_layer + post_ffn_layer
                post_ffn_layer += residual_weighted_layer
            else:
                post_ffn_layer = post_ln2_layer
                output_tensor = post_ln2_states
                #post_ffn_layer = None
            ...
            if not decompx_config.include_res:
                if decompx_config.collection_level == HEAD:
                    ...
                    output_tensor = attention_output_dense.sum(1)
            # otherwise, all other includes are taken care of.
            # the correct attribute tensor is in post_ffn_layer
            # despite the name.
            else:
                attribution_tensor = post_ffn_layer
...
class BertEncoderViT(nn.Module):
    def __init__(self, config, decompx_config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayerViT(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
        self.decompx_config = decompx_config
    ...
    def forward(
        ...
        if self.decompx_config and self.decompx_config.output_all_layers:
            # modified by Artem Vysogorets
            all_decompx_outputs = DecompXOutput(
                #attention=() if decompx_config.output_attention else None,
                #res1=() if decompx_config.output_res1 else None,
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(
                    ...
                    past_key_value,
                    output_attentions,
                    self.decompx_config # added by Fayyaz / Modarressi
                )
            ...
            # added by Fayyaz / Modarressi # modified by Artem Vysogorets
            if self.decompx_config:
                decompx_output = layer_outputs[1]
                
                encoder_scores = decompx_output.layer[0][0]
                if self.decompx_config.aggregation_in_encoder == "rollout":
                    bin_mask = torch.ones(encoder_scores.shape[-1], encoder_scores.shape[-1]).to(encoder_scores.device)
                    encoder_scores *= bin_mask
                    if aggregated_encoder_scores is None: # first layer
                        aggregated_encoder_scores = encoder_scores
                    else:
                    ...
                    aggregated_encoder_scores = torch.nan_to_num(((aggregated_encoder_scores - mean) / std)*bin_mask, 0)
                    decompx_output.aggregated = (aggregated_encoder_scores,)
                elif self.decompx_config.aggregation_in_encoder == "vector":
                    aggregated_encoder_vectors = decompx_output.layer[0][1]
                    decompx_output.aggregated = (encoder_scores,)
                else:
                    decompx_output.aggregated = (encoder_scores,)

                if self.decompx_config.output_all_layers:
                    # modified by Artem Vysogorets
                    #all_decompx_outputs.attention = all_decompx_outputs.attention + decompx_output.attention if decompx_config.output_attention else None
                    #all_decompx_outputs.res1 = all_decompx_outputs.res1 + decompx_output.res1 if decompx_config.output_res1 else None
                    #all_decompx_outputs.LN1 = all_decompx_outputs.LN1 + decompx_output.LN1 if decompx_config.output_LN1 else None
        ...
        if not return_dict:
            ...
            return tuple(
                ...
                for v in [
                    ...
                    all_self_attentions,
                    all_cross_attentions,
                    decompx_output if self.decompx_config else None,
                    all_decompx_outputs
                ]
                if v is not None # This is why we take the last element in DecompX (when output_all_layers is None). In that case, the last element is decompx_output.
            )
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        if decompx_config and decompx_config.include_classifier_w_pooler:
            # We should never enter here so don't bother
            decompx_idx = -2 if decompx_config.output_all_layers else -1
