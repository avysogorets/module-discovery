...
typedef struct {
   ...
   CPXNNZ matnz;        /* Linear constraint matrix info. */
   CPXNNZ  *cmatbeg;    /* Linear constraint matrix info. */
   CPXDIM  *cmatind;    /* Linear constraint matrix info. */
   double *cmatval;  /* Linear constraint matrix info. */
   CPXDIM cols;         /* Number of variables. */
   CPXDIM lrows;        /* Nmber of linear rows. */
   CPXDIM qrows;        /* Nmber of quadratic rows. */
                ...
                void *cbhandle)
{
   ...
   status = CPXXcallbackgetrelaxationpoint (context, relx, 0, data->cols - 1, &objval);
   if ( status ) {
   ...
   double zerotol = -1.0;
   status = CPXXcallbackgetrelaxationpi (context, lrpi, 0, data->lrows - 1, 
   ...
   if ( bestvar > 0 ) {
      ...
      status = CPXXcallbackmakebranch (context, 1, &bestvar, "L", &up,
      ...
      status = CPXXcallbackmakebranch (context, 1, &bestvar, "U", &down,
                                       0, 0, NULL, NULL, NULL, NULL, NULL,
                                       objval, &downchild);
      if ( status ) {
...
main (int argc, char *argv[])
{
   ...
   for (a = 1; a < argc; ++a) {
      ...
      status = CPXXsetdblparam (env, CPXPARAM_TimeLimit, 120);
      if ( status ) {
      ...
      data.lrows = CPXXgetnumrows (env, lp);
      data.qrows = CPXXgetnumqconstrs (env, lp);
      data.objsen = CPXXgetobjsen (env, lp);
      ...
      status = CPXXgetobj (env, lp, data.obj, 0, data.cols - 1);
      if ( status ) {
      ...
      CPXXgetrhs (env, lp, data.rhs, 0, data.lrows - 1);

      CPXNNZ matspace = CPXXgetnumnz (env, lp);
      data.cmatbeg = malloc (matspace * sizeof (CPXNNZ));
      data.cmatind = malloc (matspace * sizeof (CPXDIM));
      data.cmatval = malloc (matspace * sizeof (double));
      CPXNNZ surplus;
      status = CPXXgetcols (env, lp, &data.matnz, 
      ...
      for (q = 0; q < data.qrows; ++q) {
         ...
         CPXDIM lsp;
         CPXNNZ qsp;
         status = CPXXgetqconstr (env, lp, &qc->lnzc, &qc->qnzc, &rhs, &sense, 
         ...
         qc->lnzc = - lsp;
         qc->qnzc = - qsp;
         qc->lind = malloc (qc->lnzc * sizeof(CPXDIM));
         qc->lval = malloc (qc->lnzc * sizeof(double));
         qc->qrow = malloc (qc->qnzc * sizeof(CPXDIM));
         qc->qcol = malloc (qc->qnzc * sizeof(CPXDIM));
