...
ILOCPCONSTRAINTWRAPPER4(IloMyDistanceConstraint, cp,
  IloIntVarArray2, a,
...
void NetworkOptim(const char* filename, Mode mode) {
  ...
  try {
    ...
    for (IloInt i = 0; i < n; i++) {
      ...
      for (IloInt j = 0; j < n; j++) {
        char name[100];
        snprintf(name, sizeof(name), "A[%ld][%ld]", (long)i, (long)j);
        a[i].add(IloIntVar(env, 0, possibleArcs[i][j], name));
      }
    }
    ...
    for (IloInt i = 0; i < n; i++) {
      ...
      for (IloInt j = 0; j < n; j++) {
        char name[100];
        snprintf(name, sizeof(name), "D[%ld][%ld]", (long)i, (long)j);
        if (i == j)
          dist[i].add(IloIntVar(env, 0, 0, name));
        else
          dist[i].add(IloIntVar(env, 0, maxDistances[i][j], name));
      }
      mdl.add(IloInferred(env, dist[i])); // not a decision variable
    }
    ...
    IloIntExprArray nbNeighbors(env, n);
    for (IloInt i = 0; i < n; i++) {
      nbNeighbors[i] = IloSum(a[i]) - 1; // a[i][i] is = 1
      mdl.add(nbNeighbors[i] >= 1);
    }
