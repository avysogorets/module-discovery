...
IloModel ReadModel(
  ...
  char name[100];
  for (i = 0; i < nbTasks; i++) {
    snprintf(name, sizeof(name), "ACT%ld", (long)i);
  ...
  for (j = 0; j < nbResources; j++) {
    snprintf(name,sizeof(name),  "RES%ld", (long)j);
...
void runConflictRefinerWithPreferences(
  IloCP cp,
  ...
  for (IloInt i = 0; i < otherCts.getSize(); ++i) {
    cts.add(otherCts[i]);
    prefs.add(1.0); // Normal preference
  }
  for (IloInt i = 0; i < preferredCts.getSize(); ++i) {
...
void runConflictRefinerAllConflicts(IloCP cp, IloConstraintArray cts) {
  ...
  while (0 < forbiddenCts.getSize()) {
    ...
    for (IloInt i = 0; i < forbidden.getSize(); ++i) {
