...
int main(int, const char*[]) {
  ...
  try {
    ...
    for (IloInt i = 0; i < NbTasks; ++i) {
      ...
      IloInt d    = TaskDur [i];
      tp[i] = type;
      snprintf(name, sizeof(name), "A%ld_TP%ld", (long)i, (long)type);
      a[i] = IloIntervalVar(env, d, name);
      IloIntervalVarArray alt(env, 2);
      snprintf(name, sizeof(name), "A%ld_M1_TP%ld", (long)i, (long)type);
      a1[i] = IloIntervalVar(env, name);
      a1[i].setOptional();
      alt[0] = a1[i];
      snprintf(name, sizeof(name), "A%ld_M2_TP%ld", (long)i, (long)type);
    ...
    for (IloInt i = 0; i < NbTasks; ++i) {
      ...
      for (IloInt j = 0; j < NbTypes; ++j) {
        isLongSetup1[j] = (30 <= SetupM1[NbTypes*tpi+j]);
        isLongSetup2[j] = (30 <= SetupM2[NbTypes*tpi+j]);
      }
