...
"Postpone" decision:
  ...
  - if the selection is valid, branch between postponing or setting as absent.
...
IloBool SetTimesSearch::computeLocalCut(
  IloInt* localcut,
  IlcIntervalVar var,
  IloInt date
) {
...
IloBool SetTimesSearch::applyLocalCut(
  IloInt* localcut,
  IlcIntervalVar var,
  IloInt date
) {
...
int main(int argc, const char* argv[]) {
  ...
  try {
    ...
    for (i = 0; i < nbTasks; i++) {
      ...
      snprintf(buffer, sizeof(buffer), "T%ld", (long)i);
    ...
    for (i = 0; i < nbTasks; i++) {
      ...
      if (nbModes == 1)
        ...
        for (k = 0; k < nbModes; ++k) {
          ...
          snprintf(buffer, sizeof(buffer), "T%ldM%ld", (long)i, (long)k);
          alt.setName(buffer);
          alt.setOptional();
          altModes.add(alt);
          intervals.add(alt);
