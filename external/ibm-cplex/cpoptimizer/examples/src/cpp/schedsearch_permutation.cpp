...
ILCCTDEMON0(DeltaHead1Demon, SamePermutationCtI, propagateDeltaHead1)
ILCCTDEMON0(DeltaHead2Demon, SamePermutationCtI, propagateDeltaHead2)
ILCCTDEMON0(DeltaTail1Demon, SamePermutationCtI, propagateDeltaTail1)
ILCCTDEMON0(DeltaTail2Demon, SamePermutationCtI, propagateDeltaTail2)
ILCCTDEMON0(DeltaPresence1Demon, SamePermutationCtI, propagateDeltaPresence1)
ILCCTDEMON0(DeltaPresence2Demon, SamePermutationCtI, propagateDeltaPresence2)
ILCCTDEMON0(NotSequenced1Demon, SamePermutationCtI, propagateNotSequenced1)
ILCCTDEMON0(NotSequenced2Demon, SamePermutationCtI, propagateNotSequenced2)
...
void SamePermutationCtI::skipOne(
  IlcIntervalSequenceVar s1,
  IlcIntervalVar ti1,
  IlcIntervalSequenceVar s2,
  IlcIntervalVar ti2
...
void SamePermutationCtI::extendHead(
  IlcIntervalSequenceVar s1,
  IlcIntervalVar ti1
) {
...
void SamePermutationCtI::extendTail(
  IlcIntervalSequenceVar s1,
  IlcIntervalVar ti1
) {
...
void SamePermutationCtI::skipOneFrom(
  IlcIntervalSequenceVar s,
  IlcIntervalVar ti,
  IlcIntervalSequenceVar s1,
  IlcIntervalVar ti1,
...
void SamePermutationCtI::checkPartialChain(
  IlcIntervalVar v1,
  IlcIntervalVar earl1,
  IlcIntervalVar latr1,
  IlcIntervalSequenceVar s1,
...
#ifndef NDEBUG
void ValidateCheckHeadNeighbour(
  IlcIntervalSequenceVar s,
  IlcIntervalVar v,
  IlcIntervalVar head
) {
...
void SamePermutationCtI::checkHeadNeighbour(
  IlcIntervalSequenceVar s1,
  IloInt index
) {
  ...
  if (s2.isInHead(v2))
    checkPartialChain(
      v1, s1.getOneEarlierInHead(v1), s1.getOneLaterInHead(v1), s1,
      v2, s2.getOneEarlierInHead(v2), s2.getOneLaterInHead(v2), s2);
}
...
#ifndef NDEBUG
void ValidateCheckTailNeighbour(
  IlcIntervalSequenceVar s,
  IlcIntervalVar v,
  IlcIntervalVar tail
) {
...
void SamePermutationCtI::checkTailNeighbour(
  IlcIntervalSequenceVar s1,
  IloInt index
 ) {
  // This function allows to check the relative position of
  ...
  if (s2.isInTail(v2))
    checkPartialChain(
      v1, s1.getOneEarlierInTail(v1), s1.getOneLaterInTail(v1), s1,
      v2, s2.getOneEarlierInTail(v2), s2.getOneLaterInTail(v2), s2
    );
}
...
//   same presence than its companion in s1
void SamePermutationCtI::propagateNotSequenced(IlcIntervalSequenceVar s1) {
  assert(allSamePresence());
  IlcIntervalSequenceVar s2 = getOtherSequence(s1);
  if (s1.isSequenced() || s2.isSequenced())
    return;
  if ((getHeadInterval(s2).getImpl() == s2.getLatestInHead().getImpl())) {
    for (IlcIntervalSequenceVar::Iterator
  ...
  if ((getTailInterval(s2).getImpl() == s2.getLatestInTail().getImpl())) {
    for (IlcIntervalSequenceVar::Iterator
         it(s1, IlcIntervalSequenceVar::NotSequenced);
         it.ok(); ++it) {
      IlcIntervalVar var = *it;
...
void SamePermutationCtI::propagateHead(
  IlcIntervalSequenceVar s,
  IlcIntervalVar from
) {
  syncHead();
...
void SamePermutationCtI::propagateTail(
  IlcIntervalSequenceVar s,
  IlcIntervalVar from
) {
  syncTail();
...
void SamePermutationCtI::propagatePresence(
  IlcIntervalSequenceVar s,
  IlcIntervalVar var
) {
  if (s.isInHead(var) &&
...
void SamePermutationCtI::display(std::ostream& str) const {
  if (getName())
    str << getName();
  else
...
void MakeConveyorBelt(
  ...
  for (IloInt i = 0; i < nb; ++nb) {
    ...
    trans.setOptional();
    snprintf(buffer, sizeof(buffer), "CB<Item%ld>", (long)i);
    trans.setName(buffer);
    intervals.add(trans);

    IloIntervalVar load(env, loadTime);
    load.setOptional();
    snprintf(buffer, sizeof(buffer), "Load<Item%ld>", (long)i);
    load.setName(buffer);
    loads.add(load);

    IloIntervalVar unload(env, unloadTime);
    unload.setOptional();
    snprintf(buffer,sizeof(buffer), "Unload<Item%ld>", (long)i);
  ...
  try {
    ...
    for (IloInt i = 0; i < nbJobs; i++) {
      ...
      for (IloInt j = 0; j < nbMachines; j++) {
        ...
        snprintf(name, sizeof(name), "Job%ldShop%ld", (long)i, (long)j);
        ti.setName(name);
        machines[j].add(ti);
        if (0 != prec.getImpl())
    ...
    for (IloInt j = 0; j < nbMachines; j++) {
      ...
      snprintf(name, sizeof(name), "Shop%ld", (long)j);
      var.setName(name);
      sequences.add(var);
