...
stops:
  - The intervals are either operations or alternatives of operations on a
    machine. The scheduling phase only considers the intervals that are
    present and sequenced.
  - Because of the very simple structure of the problem, we choose to use a
    greedy algorithm which consists in fixing the start of the sequenced
    intervals at their lower bounds. Industrial problems may require a more
    sophisticated chronological scheduling algorithm such as a variant of the
    setTimes algorithm.
  - The horizon is given by the control parameters that is used to stop the
    sequencing phase. The parameter value is equivalent to about fifteen
...
In the Flexible Job-Shop Scheduling problem sample, we learn
  ...
  - How to interleave it with a chronological scheduling building algorithm.
...
SequenceHeadSearch::SequenceHeadSearch(
  IloCPEngine cp,
  IloIntervalSequenceVarArray sequences,
  IloIntVar makespan
)
...
IloBool SequenceHeadSearch::select(
  IlcIntervalSequenceVar &s,
  IlcIntervalVar &var
) const {
...
void SequenceHeadSearch::complete(IloInt boundary) {
