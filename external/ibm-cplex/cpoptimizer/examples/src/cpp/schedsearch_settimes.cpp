...
DefineModel(
  ...
  for (j = 0; j < numberOfResources; j++) {
    ...
    cumuls.add(cumul);
    snprintf(buffer, sizeof(buffer), "R%ld", (long)j);
  ...
  for (j = 0; j < numberOfJobs; j++) {
    ...
    jobs->add(var);
    snprintf(buffer, sizeof(buffer), "J%ld", (long)j);
    var.setName(buffer);
...
SetTimesDominance::SetTimesDominance(IloCPEngine cp, RCPSPScheduler* s)
  : _intervals(IlcIntervalVarArray(cp, s->getJobs()->getIntervals().getSize()))
  , _postponeDates(0) {
...
ILCGOAL3(PostponeSetTimesDominanceGoal,
  SetTimesDominance*, s,
  IloInt, p,
  IloInt, date
) {
...
SetTimesResources::SetTimesResources(IloCPEngine cp, RCPSPScheduler* s)
  : _intervals(
      IlcIntervalVarArray(cp, s->getJobs()->getIntervals().getSize())
    )
  ...
  for (IloInt i = 0; i < nbOfIntervals; ++i) {
    ...
    if (nbOfResources > 0) {
      ...
      for (IloInt r = 0; r < nbOfResources; ++r)
  ...
  for (IloInt r = 0; r < _nbOfResources; ++r) {
    ...
    if (resNbOfIntervals > 0) {
      ...
      for (IloInt i = 0; i < resNbOfIntervals; ++i)
...
ILCGOAL3(PostponeSetTimesResourcesGoal,
  SetTimesResources*, s,
  IloInt, p,
  IloInt, date
) {
...
void RcpspProblemSample(
  ...
  try {
    ...
    IloModel model = DefineModel(
      env, numberOfJobs, numberOfResources, resourceCaps, resourceUsage,
      durations, precedences, jobs, resources, makespan
    );
    IloCP cp(model);
...
int main(int, const char*[]) {
  ...
  InitParameters(
    numberOfJobs, numberOfResources, resourceUsage, durations, precedences
  );
