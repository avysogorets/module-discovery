  ...
  try {
    ...
    for (IloInt i = 0; i < nbWeeks; i++) {
      for (IloInt j = 0; j < nbGamesPerWeek; j++) {
        ...
        vars.add(away[i][j]);
        vars.add(games[i][j]);
        model.add(IloAllowedAssignments(env, vars, hag));
      }
    }
    //
    ...
    IloIntVar breaks(env, n - 2, n * (nbWeeks / 2), "breaks");
