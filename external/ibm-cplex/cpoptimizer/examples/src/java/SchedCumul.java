...
public class SchedCumul {
    ...
    private IloCumulFunctionExpr workersUsage;
    private IloCumulFunctionExpr cash;
    ...
    static String timeString(int t) {
        if (t == IloCP.IntervalMin)
            return "IntervalMin";
        else if (t == IloCP.IntervalMax)
            return "IntervalMax";
        else
            return "" + t;
    }
    ...
    public void run() {
        ...
        try {
            ...
            makeHouse(cp, 0,  31, ends, allTasks);
            makeHouse(cp, 1,   0, ends, allTasks);
            makeHouse(cp, 2,  90, ends, allTasks);
            makeHouse(cp, 3, 120, ends, allTasks);
            makeHouse(cp, 4,  90, ends, allTasks);

            cp.add(cp.le(0, cash));

            cp.add(cp.le(workersUsage, nbWorkers));

            cp.add(cp.minimize(cp.max(exprFromList(ends))));

            /* EXTRACTING THE MODEL AND SOLVING. */
            cp.setParameter(IloCP.IntParam.FailLimit, 10000);
            if (cp.solve()) {
                System.out.println(
                    "Solution with objective " + (int)cp.getObjValue() + ":"
                );
                ...
                for (int i = 0; i < segs; i++) {
                    System.out.println(
                        ...
                        " in [" + timeString(cp.getSegmentStart(cash, i)) +
                        ", " + timeString(cp.getSegmentEnd(cash, i)) + ")"
                    );
                }
            } else {
