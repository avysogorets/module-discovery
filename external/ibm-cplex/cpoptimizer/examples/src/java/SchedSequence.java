...
public class SchedSequence {
    ...
    public static IloNumExpr tardinessCost(IloCP cp, IloIntervalVar task, int dd, double weight) throws IloException {
        return cp.prod(weight, cp.max(0, cp.diff(cp.endOf(task), dd)));
    }


    public static IloNumExpr makeHouse(
        IloCP cp,
        IloNumExpr cost,
        ...
        for (int i = 0; i < nbTasks; i++ ) {
            tasks[i] = cp.intervalVar(taskDurations[i], name + "-" + taskNames[i]);
        ...
        cost = cp.sum(cost, tardinessCost(cp, house, dd, weight));
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            cost = makeHouse(cp, cost, allTasks, joeTasks, jimTasks, joeLocations, jimLocations, 0, 0,   120, 100.0);
            cost = makeHouse(cp, cost, allTasks, joeTasks, jimTasks, joeLocations, jimLocations, 1, 0,   212, 100.0);
            cost = makeHouse(cp, cost, allTasks, joeTasks, jimTasks, joeLocations, jimLocations, 2, 151, 304, 100.0);
            cost = makeHouse(cp, cost, allTasks, joeTasks, jimTasks, joeLocations, jimLocations, 3, 59,  181, 200.0);
            cost = makeHouse(cp, cost, allTasks, joeTasks, jimTasks, joeLocations, jimLocations, 4, 243, 425, 100.0);
            ...
            if (cp.solve()) {
                System.out.println("Solution with objective " + (int)cp.getObjValue() + ":");
