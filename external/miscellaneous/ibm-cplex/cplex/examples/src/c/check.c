 ...
 * Copyright IBM Corporation 1997, 2024. All Rights Reserved.
 *
                 ...
                 char         **sosname)
{
   ...
   for (i = numsos-1; i >= 0; i--) {
      ...
      snprintf (sosindstr, 32, "(sosind + %lld)", (long long) sosbeg[i]);
      snprintf (soswtstr, 32, "(soswt + %lld)", (long long) sosbeg[i]);
                 ...
                 char         **colname)
{
   ...
   for (i = ccnt - 1, k = nzcnt; i >= 0; i--) {
      collen = k - cmatbeg[i];
      k      = cmatbeg[i];
      snprintf (buffer, 64, "cmatval for column %lld", (long long) i);
                 ...
                 char **rowname)
{
   ...
   for (i = rcnt - 1, k = nzcnt; i >= 0; i--) {
      rowlen = k - rmatbeg[i];
      k      = rmatbeg[i];
      snprintf (buffer, 64, "rmatval for row %lld", (long long) i);
                    ...
                    CPXCHANNELptr reschan)
{
   ...
   i = 3;
   j = snprintf (buffer, 256, "i = %d", i);
   if ( j != 5 ) {
      display (errorchan,
               "Possible name conflict with 'snprintf' C library function\n");
      status = FAIL;
   }
   j = sscanf ("45", "%d", &i);
   if ( j != 1 || i != 45 ){
