 ...
 * Copyright IBM Corporation 2007, 2024. All Rights Reserved.
 *
               ...
               int convex)
{
    ...
    * recipient j */
   for (i = 0; i < SUPPLY_X_DEMAND; i++) {
      snprintf((char *)names[i], NAME_BUF_SIZE, "x%d", i+1);
      cost[i] = 0.0;
   }
    ...
    * this shipment. */
   for (i = 0; i < SUPPLY_X_DEMAND; i++) {
      snprintf((char *)names[i], NAME_BUF_SIZE, "y%d", i+1);
      cost[i] = 1.0;
   }
   status = CPXXnewcols (env, lp, SUPPLY_X_DEMAND, cost,
                         NULL, NULL, NULL, names);
   ...
   for (i = 0; i < SUPPLY_X_DEMAND; i++) {
      snprintf((char *)names[i], NAME_BUF_SIZE, "p%d", i+1);
   }
