...
const IloInt TaskDurations [] = { 35, 15, 40, 15, 5, 10, 5, 10, 5, 5, };
...
void MakeHouse(
  ...
  for (IloInt i = 0; i < NbTasks; ++i) {
    char name[100];
    snprintf(name, sizeof(name), "H%ld-%s", (long)id, TaskNames[i]);
  ...
  try {
    ...
    if (cp.solve()) {
      ...
      for (IloInt h = 0; h < NbHouses; h++) {
        for (IloInt i = 0; i < cp.getNumberOfSegments(houseState[h]); i++) {
          ...
          cp.out() << " in " << DisplayStateSegment(cp, houseState[h], i) << std::endl;
        }
      }
    }
    else {
