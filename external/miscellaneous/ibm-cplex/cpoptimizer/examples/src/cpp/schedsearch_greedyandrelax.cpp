...
To achieve this, consider the data sets:
  ...
  reversible data, search control invariants, etc.) It creates an instance
...
void MakeJob(
  ...
  top.setOptional();
  snprintf(buffer, sizeof(buffer), "Job%ld<Type%ld>", (long)oindex, (long)type);
  top.setName(buffer);
  IloIntervalVarArray spans(env);

  for (IloInt i = 0; i < numberOfWorkers; i++) {
    IloIntervalVar span(env);
    span.setOptional();
    spans.add(span);
    IloIntervalVarArray opers(env);
    Job* job = new (env) Job(type, deadline, (i == 0), top, span, opers);
    IloInt w = job->getWorkers(i);
    snprintf(buffer, sizeof(buffer), "Job%ld<Type%ldWorker%ld>", (long)oindex, (long)type, (long)w);
    span.setName(buffer);
    jobs.add((IloAny) job);
    IloInt a = job->getAllowedAbsent();
    IloInt numberOfOpers = job->getNumberOfOpers();
    for (IloInt j = 0; j < numberOfOpers; j++) {
      IloIntervalVar var(env, 1);
      var.setOptional();
      opers.add(var);
      snprintf(buffer, sizeof(buffer), "Oper%ld<Job%ldType%ldWorker%ld>",
...
IloModel DefineModel(
  ...
  for (IloInt j = 0; j < numberOfJobsA; j++)
    MakeJob(model, workers, j, 0, deadlinesA[j], jobs);
  for (IloInt j = 0; j < numberOfJobsB; j++)
...
// The interval variables are retrieved from the IloCP instance of the search.
class JobRelaxation {
private:
  Job* _job;
...
// for an iteration
ILCGOAL1(RelaxGreedySearchGoal, RelaxGreedySearch*, s) {
...
IloBool RelaxGreedySearch::select(
  IloInt& job, IloInt& interval, IloInt& date
) const {
...
int main(int, const char*[]) {
  ...
  try {
    ...
    IloModel model = DefineModel(
      env, NumberOfJobsA, NumberOfJobsB, DeadlinesA, DeadlinesB, jobs
    );
