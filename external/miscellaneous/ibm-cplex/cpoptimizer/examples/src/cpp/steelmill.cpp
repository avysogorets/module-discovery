...
int main(int, const char * []) {
  ...
  // Objective function
    ...
    // So, place all orders 0..k onto the smallest indexed slabs.
    // If order k+1 needs a new slab we need only consider the
    // lowest indexed unused slab, not all of them.
    IloIntExpr prevMax(env, -1);
    for (o = 0; o < nbOrders; o++) {
      model.add(where[o] <= 1 + prevMax);
      prevMax = IloMax(where[o], prevMax);
    }
    IloCP cp(model);
    if (cp.solve()) {
