...
public class Color {
    ...
    public static void main(String[] args) {
        ...
        try {
            cp = new IloCP();
            IloIntVar Belgium = cp.intVar(0, 3, "B");
            IloIntVar Denmark = cp.intVar(0, 3, "DK");
            IloIntVar France = cp.intVar(0, 3, "F");
            IloIntVar Germany = cp.intVar(0, 3, "D");
            IloIntVar Luxembourg = cp.intVar(0, 3, "L");
            IloIntVar Netherlands = cp.intVar(0, 3, "NL");
            ...
            if (cp.solve()) {
                ...
                System.out.println(
                    "Belgium:     " + Names[(int)cp.getValue(Belgium)]
                );
                System.out.println(
                    "Denmark:     " + Names[(int)cp.getValue(Denmark)]
                );
                System.out.println(
                    "France:      " + Names[(int)cp.getValue(France)]
                );
                System.out.println(
                    "Germany:     " + Names[(int)cp.getValue(Germany)]
                );
                System.out.println(
                    "Luxembourg:  " + Names[(int)cp.getValue(Luxembourg)]
                );
                System.out.println(
                    "Netherlands: " + Names[(int)cp.getValue(Netherlands)]
                );
            }
        }
