...
- Scenario 4: Find a minimal conflict partition that is, a set of disjoint
                            minimal conflicts S1,...,Sn such that when all
                            constraints in S1 U S2 U... U Sn are removed
                            from the model: the model becomes feasible.
...
public class SchedConflict {
    private IloConstraint[] capacityCts;
    private IloConstraint[] precedenceCts;
    ...
    static IloIntExpr[] exprFromList(List<IloIntExpr> list) {
        return (IloIntExpr[]) list.toArray(new IloIntExpr[list.size()]);
    }

    static IloConstraint[] ctFromList(List<IloConstraint> list) {
        return (IloConstraint[]) list.toArray(new IloConstraint[list.size()]);
    }

    //----- RCPSP Model creation --------------------------------------------

    private void buildModel(IloCP cp, String fileName) throws IOException {
        ...
        try {
            ...
            List<IloConstraint> capacityList = new ArrayList<IloConstraint>();
            List<IloConstraint> precedenceList = new ArrayList<IloConstraint>();
            IloCumulFunctionExpr[] resources =
                new IloCumulFunctionExpr[nbResources];
            ...
            for (int i = 0; i < nbTasks; i++) {
                ...
                for (int s = 0; s < nbSucc; s++ ) {
                    ...
                    cp.add(pct);
                    precedenceList.add(pct);
                }
            }
            for (int j = 0; j < nbResources; j++) {
                resources[j].setName("RES" + j);
                IloConstraint cct = cp.le(resources[j], capacities[j]);
                cp.add(cct);
                capacityList.add(cct);
            }
            precedenceCts = ctFromList(precedenceList);
            capacityCts   = ctFromList(capacityList);
            IloObjective objective = cp.minimize(cp.max(exprFromList(ends)));
            cp.add(objective);
        }
    ...
    private static void runBasicConflictRefiner(IloCP cp) throws IloException {
    ...
    private static void runConflictRefinerWithPreferences(
            IloCP cp,
            IloConstraint[] preferredCts,
            IloConstraint[] otherCts
    ) throws IloException {
    ...
    private static void runConflictRefinerPartition(
        IloCP cp,
        IloConstraint[] cts
    ) throws IloException {
        int n = cts.length;
        ...
        while (cp.refineConflict(cts, prefs)) {
            ...
            cp.writeConflict();
            for (int i = 0; i < n; ++i) {
                if (cp.getConflict(cts[i]) ==
                    IloCP.ConflictStatus.ConflictMember
                ) {
                    // Next run will ignore constraints of the current conflict
                    prefs[i] = -1.0;
                }
            }
        }
    ...
    private static void runConflictRefinerAllConflicts(
        IloCP cp,
        IloConstraint[] cts
    ) throws IloException {
        int n = cts.length;
        double[] prefs = new double[n];
        ArrayList<ArrayList<Integer>> forbiddenCts =
        ...
        while (0<forbiddenCts.size()) {
            ArrayList<Integer> forbidden =
                (ArrayList<Integer>)forbiddenCts.remove(0);
            for (int i = 0; i < n; ++i) {
            ...
            if (cp.refineConflict(cts, prefs)) {
                ...
                for (int i = 0; i < n; ++i) {
                    if (cp.getConflict(cts[i]) ==
                        ...
                        ArrayList<Integer> newforbidden =
    ...
    public void run(String fileName, int failLimit) throws IOException {
        IloCP cp = null;
        try {
            cp = new IloCP();
            buildModel(cp, fileName);
            int nbCapacityCts   = capacityCts.length;
            int nbPrecedenceCts = precedenceCts.length;
            int nbCts = nbCapacityCts + nbPrecedenceCts;
            IloConstraint[] allCts = new IloConstraint[nbCts];
            ...
            cp.setParameter(
                IloCP.IntParam.CumulFunctionInferenceLevel,
                IloCP.ParameterValues.Extended
            );
            cp.setParameter(
                IloCP.IntParam.ConflictRefinerOnVariables,
            ...
            if (cp.solve()) {
                ...
                System.out.println(
                    "Solution found with makespan : " + cp.getObjValue()
                );
            } else {
                if (cp.getInfo(IloCP.IntInfo.SearchStatus) !=
                    IloCP.ParameterValues.SearchCompleted.getValue()) {
                    // No solution found but problem was not proved infeasible
                    System.out.println(
                        "No solution found but problem " +
                        " was not proved infeasible."
                    );
                } else {
                    // Run conflict refiner if problem was proved infeasible
                    System.out.println(
                        "Infeasible problem, running conflict refiner ..."
                    );
                    System.out.println();
                    System.out.println("SCENARIO 1: Basic conflict refiner:");
                    runBasicConflictRefiner(cp);
                    cp.setParameter(
                        IloCP.IntParam.LogVerbosity,
                        IloCP.ParameterValues.Quiet
                    );
                    System.out.println();
                    System.out.println(
                        "SCENARIO 2: Conflict refiner with preference " +
                        "on resource capacity constraints:"
                    );
                    runConflictRefinerWithPreferences(
                        cp, capacityCts, precedenceCts
                    );
                    System.out.println();
                    System.out.println(
                        "SCENARIO 3: Conflict refiner with preference " +
                        "on precedence constraints:"
                    );
                    runConflictRefinerWithPreferences(
                        cp, precedenceCts, capacityCts
                    );
                    System.out.println();
    ...
    public static void main(String[] args) throws IOException {
        ...
        SchedConflict sc = new SchedConflict();
        sc.run(fileName, failLimit);
    }
}
