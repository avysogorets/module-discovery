...
public class SchedGoalProg {
    ...
    public SchedGoalProg(IloCP solver, String fileName) throws IOException, IloException {
        cp = solver;
    ...
    private void createModel(String dataFile) throws IOException, IloException {
        ...
        for (int i = 0; i < nbJobs; i++) {
            IloIntervalVar job = cp.intervalVar("Op " + i);
            job.setStartMin(rel[i]);
            job.setEndMax(due[i]);
            IloIntervalVar[] jobm = new IloIntervalVar[nbMachines];
            for (int j = 0; j < nbMachines; j++) {
                String name = "Alt" + i + "_" + j + "_C" + costs[j][i];
                jobm[j] = cp.intervalVar(dur[j][i], name);
                jobm[j].setOptional();
                machines[j][i] = jobm[j];
            }
    ...
    public double solveGoalProgramming(int failLimit) throws IloException {

        // Objective 1
        System.out.println();
        System.out.println(" ! ---------------------------------------------");
        System.out.println(" ! STEP 1: Minimizing use of expensive resources");
        System.out.println(" ! ---------------------------------------------");
        ...
        cp.setParameter(
            ...
            IloCP.ParameterValues.Terse
        );
        cp.setParameter(IloCP.IntParam.FailLimit, failLimit / 2);
        cp.setParameter(
            IloCP.IntParam.NoOverlapInferenceLevel,
            IloCP.ParameterValues.Extended
        );
        cp.solve();
        int cost1 = (int)cp.getObjValue();
        int failsLeft = failLimit - cp.getInfo(IloCP.IntInfo.NumberOfFails);
        ...
        System.out.println();
        System.out.println(" ! -------------------------------------------");
        System.out.println(" ! STEP 2: Minimizing cost of normal resources");
        System.out.println(" ! -------------------------------------------");
        cp.remove(obj1);
        ...
        for (int i = 0; i < nbJobs; i++) {
            for (int j = 0; j < nbMachines; j++) {
                if (costs[j][i] < 1000)
                    costExpr2 = cp.sum(
                        costExpr2,
                        cp.prod(costs[j][i], cp.presenceOf(machines[j][i]))
                    );
            }
        }
        IloObjective obj2 = cp.minimize(costExpr2);
        cp.add(obj2);
        cp.setParameter(IloCP.IntParam.FailLimit, failsLeft);
        cp.setStartingPoint(startSol);
        cp.solve();
        int cost2 = (int)cp.getObjValue();
        return 1000.0 * cost1 + cost2;
    }

    public double solveBasic(int failLimit) throws IloException {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            SchedGoalProg gp = new SchedGoalProg(cp, fileName);
            if (useGoalProgramming) {
                System.out.println("Solving using goal programming ...");
                cost = gp.solveGoalProgramming(failLimit);
            }
            else {
                System.out.println("Solving in a single step  ...");
                cost = gp.solveBasic(failLimit);
            }

            System.out.println();
            System.out.println(" ! -------------------------------");
            System.out.println(" ! Cost = " + (long)cost);
            System.out.println(" ! -------------------------------");
        }
