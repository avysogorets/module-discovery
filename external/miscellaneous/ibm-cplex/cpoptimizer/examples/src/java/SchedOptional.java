...
public class SchedOptional {
    ...
    public static IloIntExpr makeHouse(
        IloCP cp,
        IloIntExpr skill,
        IntervalVarList allTasks,
        ...
        cp.add(cp.add(cp.eq(cp.presenceOf(taskMatrix[masonry][joe]),
                            cp.presenceOf(taskMatrix[carpentry][joe]))));
        cp.add(cp.add(cp.eq(cp.presenceOf(taskMatrix[roofing][jack]),
                            cp.presenceOf(taskMatrix[facade][jack]))));
        cp.add(cp.add(cp.eq(cp.presenceOf(taskMatrix[carpentry][joe]),
                            cp.presenceOf(taskMatrix[roofing][joe]))));
        cp.add(cp.add(cp.eq(cp.presenceOf(taskMatrix[garden][jim]),
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int h = 0; h < nbHouses; h++) {
                skill = makeHouse(cp, skill, allTasks, workerTasks, h, deadline);
            }
