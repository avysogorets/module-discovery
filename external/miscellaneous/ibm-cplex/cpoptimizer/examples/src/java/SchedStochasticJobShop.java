...
public class SchedStochasticJobShop {
    ...
    static IloIntExpr MakeScenarioSubmodel(
            IloCP cp,
            int nbJobs,
            int nbMachines,
            int[][] machines,
            int[][] durations,
            IloIntervalSequenceVar[] sequences
    ) throws IloException {
        ...
        for (int i = 0; i < nbJobs; i++) {
            ...
            for (int j = 0; j < nbMachines; j++) {
                IloIntervalVar ti = cp.intervalVar(durations[i][j], "J" + i + "_O" + j);
        ...
        for (int j = 0; j < nbMachines; j++) {
            sequences[j] = cp.intervalSequenceVar(machinesOps[j].toArray(), "M" + j);
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            for (int k = 0; k < nbScenarios; k++) {
                ...
                IloIntExpr scenarioMakespan = MakeScenarioSubmodel(
                    cp, nbJobs, nbMachines, machines, durations[k], scenarioSequences
                );
            ...
            if (cp.solve()) {
                System.out.printf("Expected makespan \t: %.1f\n", cp.getObjValue());
                for (int j = 0; j < nbMachines; ++j) {
