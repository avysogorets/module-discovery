    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int i = 0; i < nbWeeks; i++) {
                home[i]  = cp.intVarArray(nbGamesPerWeek, 0, n - 1, cp.arrayEltName("H", i));
                away[i]  = cp.intVarArray(nbGamesPerWeek, 0, n - 1, cp.arrayEltName("A", i));
                games[i] = cp.intVarArray(nbGamesPerWeek, 0, nbGames - 1, cp.arrayEltName("G", i));
            }
            //
            // For each play slot, set up correspondance between game id,
            // home team, and away team
            //
            IloIntTupleSet hag = cp.intTable(3);
            int[] tuple = new int[3];
            for (int i = 0; i < n; i++) {
                tuple[0] = i;
                for (int j = 0; j < n; j++) {
            ...
            for (int i = 0; i < nbWeeks; i++) {
                for (int j = 0; j < nbGamesPerWeek; j++) {
                    ...
                    vars[1] = away[i][j];
                    vars[2] = games[i][j];
                    cp.add(cp.allowedAssignments(vars, hag));
                }
            }
            //
            ...
            IloIntVar[] allSlots = cp.intVarArray(nbGames, 0, nbGames - 1, "allS");
            for (int i = 0; i < nbWeeks; i++)
                for (int j = 0; j < nbGamesPerWeek; j++)
                    allGames[i*nbGamesPerWeek+j] = games[i][j];
            cp.add(cp.inverse(allGames, allSlots));
            IloIntVar[] weekOfGame = cp.intVarArray(nbGames, 0, nbWeeks - 1, "WoG");
            ...
            for (int i = 0; i < n; i++) {
                for (int j = i + 1; j < n; j++) {
                    int g1 = Game(i, j, n);
                    int g2 = Game(j, i, n);
                    cp.add(cp.eq(cp.ge(weekOfGame[g1], mid), cp.lt(weekOfGame[g2], mid)));
                    // Six week difference...
                    if (overlap != 0)
            ...
            for (int i = 0; i < n; i++) {
                playHome[i] = cp.intVarArray(nbWeeks, 0, 1, cp.arrayEltName("PH", i));
            ...
            //            particular team
            IloIntVar[] teamBreaks = cp.intVarArray(n, 0, nbWeeks / 2, "TB");
            for (int i = 0; i < n; i++) {
                IloIntExpr nbreaks = cp.constant(0);
                for (int j = 1; j < nbWeeks; j++)
                    nbreaks = cp.sum(nbreaks, cp.eq(playHome[i][j-1], playHome[i][j]));
                cp.add(cp.eq(teamBreaks[i], nbreaks));
            }
            IloIntVar breaks = cp.intVar(n - 2, n * (nbWeeks / 2), "breaks");
