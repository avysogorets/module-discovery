...
public class Steelmill {
    public static void main(String[] args) {
        ...
        try {
            ...
            IloIntVar[] where = cp.intVarArray(nbOrders, 0, nbSlabs-1, "W");
            IloIntVar[] load = cp.intVarArray(nbSlabs, 0, weightSum, "L");
            ...
            for (m = 0; m < nbSlabs; m++) {
                ...
                for (c = 0; c < nbColors; c++) {
                    ...
                    for (o = 0; o < nbOrders; o++)
                        if (colors[o] == c)
                            orLen++;
                    IloConstraint[] orCts = new IloConstraint[orLen];
                    orLen = 0;
                    for (o = 0; o < nbOrders; o++) {
                        if (colors[o] == c)
                            orCts[orLen++] = cp.eq(where[o], m);
                    }
                    colorExpArray[c] = cp.intExpr(cp.or(orCts));
                }
            ...
            // All slabs are identical.
            // So, place all orders 0..k into the smallest indexed slabs.
            // If order k+1 needs a new slab we need only consider the
            // very next unused slab, not all of them.
            IloIntExpr prevMax = cp.constant(-1);
            for (o = 0; o < nbOrders; o++) {
                cp.add(cp.le(cp.diff(where[o], prevMax), 1));
                prevMax = cp.max(where[o], prevMax);
            }
