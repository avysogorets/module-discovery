    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        if self.log1p:
            ...
            if self.result_log1p_key:
                _set_obs_rep(
                    adata,
                    deepcopy(_get_obs_rep(adata, layer=key_to_process)), # deepcopy by Artem Vysogorets
        ...
        if self.binning:
            ...
            binned_rows = []
            bin_edges = []
            layer_data = deepcopy(_get_obs_rep(adata, layer=key_to_process)) # deepcopy by Artem Vysogorets
