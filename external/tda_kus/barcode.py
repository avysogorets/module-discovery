import ripserplusplus as rpp # type: ignore
""" Ripser++ may be difficult to install. I managed to do it on tarchuna
    but not on the workstation. On tarchuna, you need to load cuda 11.8.
    On the workstattion, even after downloading the right cudatoolkit
    (nvcc --version 11.8 and gcc --version 11.4.0), there are still C++
    errors when building the package.
"""
import numpy as np
import itertools
...
def barcode_entropy(barcode, dim=0):
    # Modified by Artem Vysogorets (guardrails added)
    if len(barcode[dim]):
...
def matrix_to_ripser(matrix, ntokens, lower_bound=0.0, symmetrizer='max'):
    """Convert matrix to appropriate ripser++ format"""
    # Modified by Artem Vysogorets for more symmetrizer support
    matrix = cutoff_matrix(matrix, ntokens)
    matrix = (matrix > lower_bound).astype(np.int64) * matrix
    if symmetrizer == 'max':
        matrix = np.maximum(matrix.T, matrix)
    elif symmetrizer == 'min':
        matrix = np.minimum(matrix.T, matrix)
    elif symmetrizer in ['avg', 'mean']:
        matrix = np.mean(matrix.T, matrix)
    else:
        raise NotImplementedError(f'unknown symmetrizer {symmetrizer}')
    matrix = 1.0 - matrix
